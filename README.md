...
> 首先是一个好用的待办 + 打卡工具。其次，它愿意把数据交给 AI。
>
> 大多数待办 App 把你的数据锁在云端——漂亮、好用，但 **AI 够不到**。
> MyTodo 自托管、数据归你、全部通过 REST API 敞开给 AI Agent。
>
...
市面上的待办 / 打卡 App 已经够多了。但它们几乎都有一个共同的盲区：

```
传统 App：   用户 → 界面 → 云端数据库（锁死，不开放）
                              ↑
                        AI 拿不到数据 ❌

MyTodo：     用户 → 界面 → 本地 SQLite ← REST API → AI Agent ✅
                              ↑
                        数据完全开放，AI 能读能写
```

关键不是"功能更多"，而是 **"AI 够得到数据"**。别人家的 App 再花哨，AI 也只能隔着云端干瞪眼；对会接 AI Agent 的人来说，他们缺的从来不是又一个待办 App，而是**一个愿意把数据敞开给 AI 的待办 App**。这个空白，MyTodo 来填。

### 两大支柱

**1. 全自主管理** —— 你拥有全部
- 自托管，数据存本地 SQLite，不经手任何第三方
- 没有订阅费、没有账号体系、没有数据上传、没有"服务停止"风险
- 一个 `.db` 文件就是全部，随时可备份 / 迁移 / 离线
- PWA，可装到手机主屏幕，全屏离线运行

**2. 联动 AI Agent** —— 数据对 AI 敞开
- 所有数据（待办、打卡、分类、日志）都通过 REST API 读写
- 双向交互：Agent 既能**读取**（查待办、分析打卡规律）也能**写入**（创建任务、帮你打卡）
- 零锁定：标准 JSON 接口，任何能发 HTTP 请求的 Agent / 脚本 / 自动化工具都能接入

> MyTodo 提供数据和能力，Agent 提供智能。
> 读取待办 → 分析行为规律 → 主动提醒 → 甚至帮你规划下一步——这套闭环，API 已经就绪。
